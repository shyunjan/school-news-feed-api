...
보안 원칙상 현재는 Github 리포지토리에 .env 파일을 올리지 않은 상태이기 때문에 .env 파일을 작성하지 않고 로컬에서 실행하면 아래와 같은 오류가 발생할 것이다.
...
참고로 관리자를 생성하면서 다음과 같이 관리할 학교 정보도 같이 입력하여 아래 _`2.3 학교 생성`_ 단계를 생략할 수도 있다.
...
로그인은 학교관리자나 일반 유저(학생)나 모두 아래와 같이 아이디와 패스워드를 입력해서 로그인한다.
